...
import { IUpdateGoalProps, UpdateGoalProps } from './types/GoalProps';
import { rowStyle, style } from './GoalModalStyle';
import EditIcon from '@mui/icons-material/Edit';
import { toast } from 'react-toastify';


const UpdateGoalModal: React.FC<IUpdateGoalProps> = ({ isEditing, onSubmit, title, subtitle, goalData}) => {

    const { id } = goalData;

    const { register, handleSubmit, control} = useForm<UpdateGoalProps>();
    ...
    useEffect(() => {
        if (goalData) {
            setAmount(goalData.amount);
            setDescription(goalData.description);
            if (goalData.limitDate) {
                const date = new Date(goalData.limitDate);
                setLimitDate(date);
              } else {
                setLimitDate(null); 
              }
    ...
    const onSubmitHandler = async (data: UpdateGoalProps) => {
        console.log(data)
      try {
