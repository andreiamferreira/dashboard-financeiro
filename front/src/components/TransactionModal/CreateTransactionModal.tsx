...
import { CategoryType, CreateTransactionProps, ICreateTransactionProps, TransactionTypeType } from './Types/TransactionProps';
...
const CreateTransactionModal: React.FC<ICreateTransactionProps> = ({ onSubmit, title, subtitle, isEditing, accountData}) => {

    const { id: bankAccId } = accountData;

    const { register, handleSubmit, control} = useForm<CreateTransactionProps>();
...
const onSubmitHandler = async (data: CreateTransactionProps) => {
  try {
    await onSubmit({ ...data, bankAccId });
    setOpen(false);
    cleanInput();
  } catch (error) {
